...
void Timer_Init(void) 
{
        ...
        #if (CY_PSOC5A)
            Timer_CONTROL2 &= ~Timer_CTRL_MODE_MASK;
        #endif /* Clear bits in CONTROL2 only in PSOC5A */

        #if (CY_PSOC3 || CY_PSOC5LP)
            Timer_CONTROL3 &= ~Timer_CTRL_MODE_MASK;
        ...
        /* Check if One Shot mode is enabled i.e. RunMode !=0*/
            ...
            Timer_CONTROL |= 0x04u;
                ...
                Timer_CONTROL3 &= ~Timer_CTRL_RCOD_MASK;
                Timer_CONTROL3 |= Timer_CTRL_RCOD;

                /* Clear and Enable the HW enable bit in CFG2 register */
                Timer_CONTROL3 &= ~Timer_CTRL_ENBL_MASK;
        ...
        Timer_RT1 &= ~Timer_RT1_MASK;
        Timer_RT1 |= Timer_SYNC;

        /*Enable DSI Sync all all inputs of the Timer*/
        Timer_RT1 &= ~(Timer_SYNCDSI_MASK);
    ...
    Timer_WriteCounter(Timer_INIT_PERIOD);
        ...
        Timer_SetCaptureCount(Timer_INIT_CAPTURE_COUNT);
            ...
            if (!(Timer_CONTROL & Timer__B_TIMER__TM_SOFTWARE))
            {
...
void Timer_SoftwareCapture(void) 
{
    ...
    CY_GET_REG8(Timer_COUNTER_LSB_PTR);
                                   ...
                                   
{
         ...
         * CY_SET_REG16(Timer_COUNTER_LSB_PTR, counter_temp);
         */
    #else
...
void Timer_SetCaptureMode(uint8 captureMode) 
{
    ...
    captureMode = (captureMode << Timer_CTRL_CAP_MODE_SHIFT);
    captureMode &= (Timer_CTRL_CAP_MODE_MASK);

    /* Clear the Current Setting */
    Timer_CONTROL &= ~Timer_CTRL_CAP_MODE_MASK;
...
void Timer_SetTriggerMode(uint8 triggerMode) 
{
    ...
    /* Clear the Current Setting */
    Timer_CONTROL &= ~Timer_CTRL_TRIG_MODE_MASK;
...
void Timer_SetInterruptCount(uint8 interruptCount) 
{
    ...
    /* Clear the Current Setting */
    Timer_CONTROL &= ~Timer_CTRL_INTCNT_MASK;
    /* Write The New Setting */
...
{
    while(Timer_ReadStatusRegister() & Timer_STATUS_FIFONEMP)
    {
        Timer_ReadCapture();
    }
}
