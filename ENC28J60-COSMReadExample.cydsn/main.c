/*
 ...
 -----------------------------------------
 Title  : COSM Example - WebClient - Read Feed Data
 ...
 Description : This runs a webserver on the device's IP,on port 80
			   ...
			   Webclient part will download current die temperature to COSM.com,every 30 seconds,
			   via a formatted GET Query.
			   If the DNS Query works,a "DNS Worked." message will be displayed.
			   Change the Query's API Key and FEED ID(65875 in this case.) for your case.
			   For each successful reply from the COSM server,the temperature will be displayed on the LCD.
...
void main( void ){
	/*Our query*/
	unsigned char Query[]="GET /v2/feeds/65875/datastreams/Temperature.csv HTTP/1.1\r\nHost: api.cosm.com\r\nX-ApiKey:bCbTKgjgs3WzkSUBQdkeSjd4x0CSAKxPVE1OazB1NnNJRT0g\r\nUser-Agent: PSoC3\r\nContent-Type: text/csv\r\nConnection: close\r\n\r\n";
	...
	/*Copy in our query*/
	memcpy(WebClientQuery,Query,sizeof(Query));
    ...
    for(;;){
		...
		if(QSend==1){	
			...
			/*Format and send the GET Request to api.cosm.com*/
			if(DNSWorked==1){
